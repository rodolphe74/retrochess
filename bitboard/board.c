...
void init_board_from_string(Board *board, char string[64])
{
	memcpy(board->placement, string, 64);
	board->active_color = WHITE;
	board->castling = 0x1111;
	board->en_passant = -1;
	board->halfmove_clock = 0;
	...
	for (int i =  0; i < 64; i++) {
		switch(string[i]) {
			case 'P': board->pawns[WHITE] |= ((uint64_t) 1) << i; break;
			case 'N': board->knights[WHITE] |= ((uint64_t) 1) << i; break;
			case 'B': board->bishops[WHITE] |= ((uint64_t) 1) << i; break;
			case 'R': board->rooks[WHITE] |= ((uint64_t) 1) << i; break;
			case 'Q': board->queens[WHITE] |= ((uint64_t) 1) << i; break;
			case 'K': board->king[WHITE] |= ((uint64_t) 1) << i; break;
			case 'p': board->pawns[BLACK] |= ((uint64_t) 1) << i; break;
			case 'n': board->knights[BLACK] |= ((uint64_t) 1) << i; break;
			case 'b': board->bishops[BLACK] |= ((uint64_t) 1) << i; break;
			case 'r': board->rooks[BLACK] |= ((uint64_t) 1) << i; break;
			case 'q': board->queens[BLACK] |= ((uint64_t) 1) << i; break;
			case 'k': board->king[BLACK] |= ((uint64_t) 1) << i; break;
			default: break;
		}
	}
	set_occupied(board);
...
void print_board_stdout(char *label, Board *board)
{
	char *s = (char *)malloc(251);
	char p[3];
	char cr[] = {10, 13, 0};
	...
	printf("-=%s=- %c%c%s%c%c", label, 10, 13, s, 10, 13);
	free(s);
}
...
int is_ep(int from, int to, uint8_t ep[16], int color)
{

	if (color == BLACK) {
		for (int i = 0; i < 8; i++) {	// search against WHITE
			if ( (from - 7) / 8 != (from / 8) - 1 || (from - 9) / 8 != (from / 8) - 1) {
				return FALSE;
			} else if ((from-1==ep[i] || from+1==ep[i]) && (from-7==to || from-9==to) && (to%8 == ep[i]%8)) {
				printf("B TRUE %d %d - %d %d \n", from, to, (from - 7), (from - 9));
				return ep[i];
			}
		}
	} else {
		for (int i = 8; i < 16; i++) {	// search against BLACK
			if ( (from + 7) / 8 != (from / 8) + 1 || (from + 9) / 8 != (from / 8) + 1) {
				return FALSE;
			} else if ((from-1== ep[i] || from+1==ep[i]) && (from+7==to || from+9==to) && (to%8 == ep[i]%8)) {
				printf("W TRUE %d %d\n", from, to);
...
// Returns FALSE, CASTLING, EN_PASSANT if the move from-to is pseudo legal.
// e is fed with the taken pawn when EN_PASSANT case
// ep is the buffer which contains the 16 possibly EN_PASSANT pawns strokes
int
pseudo_legal_move_ep(Board *board, int from, int to, uint8_t ep[16], int *e)
{
	...
	// The move is en passant
	*e = is_ep(from, to, ep, board->active_color);
	if (*e) {
		print_board_stdout("ep", board);
		printf("%d is_ep %d %d = %d\n", board->active_color, from, to, *e);
		return EN_PASSANT;
	}
...
// handled separately.
bool
try_move_without_notation(Board *board, int from, int to, char promote_in, Board *new_board, char *capture, int en_passant)
{
	memcpy(new_board, board, sizeof(Board));
	*(get_bitboard(new_board, from)) ^= 1ULL << from;
	*(get_bitboard(new_board, from)) ^= 1ULL << to;
	...
	if (en_passant) {
		printf("EN PASSANT *****************************\n");
		*capture = new_board->placement[en_passant];
		*(get_bitboard(new_board, en_passant)) ^= 1ULL << en_passant;
		new_board->placement[en_passant] = 0;
	}
